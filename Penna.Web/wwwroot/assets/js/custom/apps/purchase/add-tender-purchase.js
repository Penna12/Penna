"use strict";
var KTTenderPurchaseAdd = function () {
    var t, n, r, b, g, p, s, pq, pp, rq, ft, sm;

    var save = () => {
        return $.ajax({
            url: "/Purchase/SavePurchase/",
            type: "POST",
            data: $('#add_tender_purchase_form').serialize() + sm
        });
    };
    ...
    return {
        init: function () {
            (r = document.querySelector("#add_tender_purchase_form"),
                t = r.querySelector("#add_tender_purchase_submit"),
                ...
                ft = r.querySelector("#Purchase_FinalBidDateTime"),
                sm = "",
                                    ...
                                    ? (t.setAttribute("data-kt-indicator", "on"),
                                        ...
                                        setTimeout((function () {
                                            t.removeAttribute("data-kt-indicator"),
                                                Swal.fire({ title: "DİKKAT", html: `Satın almak istediğiniz ürün için<br> ${ft.value}'a kadar yanıt verilmek üzere<br>seçilen tedarikçi firmalarınıza teklif talebi göndermek<br>istiyor musunuz?`, icon: "warning", buttonsStyling: !1, showCancelButton: !0, focusConfirm: !0, confirmButtonText: "Evet", cancelButtonText: "Hayır", customClass: { confirmButton: "btn btn-primary", cancelButton: "btn btn-danger" } })
                                                    .then((function (e) {
                                                        e.isConfirmed
                                                            ? (
                                                                sm = "&sendMail=1",
                                                            ...
                                                            : "cancel" === e.dismiss && save().done((function (res) {
                                                                if (res.success) {
                                                                    Swal.fire({ text: "Form basariyla gonderildi!", icon: "success", buttonsStyling: !1, confirmButtonText: "Tamam", customClass: { confirmButton: "btn btn-primary" } })
                                                                        .then((function (e1) { e1.isConfirmed && (t.disabled = !1, window.location = r.getAttribute("data-kt-redirect")) }))
                                                                } else {
...
KTUtil.onDOMContentLoaded((function () { KTTenderPurchaseAdd.init() }));