...
var KTModalBlockAdd = function () {
    ...
    var save = () => {
        return $.ajax({
            url: "/Block/Create/",
            type: "POST",
            data: $('#kt_modal_add_blocks_form').serialize()
        });
    };
    ...
    return {
        init: function () {
            table = $("#kt_blocks_table").DataTable(),
                i = new bootstrap.Modal(document.querySelector("#kt_modal_add_blocks")),
                r = document.querySelector("#kt_modal_add_blocks_form"),
                t = r.querySelector("#kt_modal_add_blocks_submit"),
                e = r.querySelector("#kt_modal_add_blocks_cancel"),
                o = r.querySelector("#kt_modal_add_blocks_close"),
                                            ...
                                            save().done((function (res) {
                                                if (res.success) {
                                                    Swal.fire({ text: "Form basariyla gonderildi!", icon: "success", buttonsStyling: !1, confirmButtonText: "Tamam", customClass: { confirmButton: "btn btn-primary" } })
                                                        .then((function (e1) { e1.isConfirmed && (r.reset(), i.hide(), t.disabled = !1, table.ajax.reload() /*window.location = r.getAttribute("data-kt-redirect")*/) }))
                                                } else {
...
KTUtil.onDOMContentLoaded((function () { KTModalBlockAdd.init() }));