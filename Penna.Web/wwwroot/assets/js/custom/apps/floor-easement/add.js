"use strict";
var KTModalFloorEasementAdd = function () {
    var s, c, o, v, f, m, table;

    var save = () => {
        return $.ajax({
            url: "/FloorEasement/Create/",
            type: "POST",
            data: $('#kt_modal_add_floors_form').serialize()
        });
    };
    ...
    return {
        init: function () {
            table = $("#kt_floor_table").DataTable(),
                m = new bootstrap.Modal(document.querySelector("#kt_modal_add_floors")),
                f = document.querySelector("#kt_modal_add_floors_form"),
                s = f.querySelector("#kt_modal_add_floors_submit"),
                c = f.querySelector("#kt_modal_add_floors_cancel"),
                o = f.querySelector("#kt_modal_add_floors_close"),
                                            ...
                                            save().done((function (res) {
                                                if (res.success) {
                                                    Swal.fire({ text: "Form basariyla gonderildi!", icon: "success", buttonsStyling: !1, confirmButtonText: "Tamam", customClass: { confirmButton: "btn btn-primary" } })
                                                        .then((function (e1) { e1.isConfirmed && (m.hide(), s.disabled = !1, table.ajax.reload() /*window.location = f.getAttribute("data-kt-redirect")*/) }))
                                                } else {
...
KTUtil.onDOMContentLoaded((function () { KTModalFloorEasementAdd.init() }));